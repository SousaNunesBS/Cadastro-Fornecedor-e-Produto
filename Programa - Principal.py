...
def  consultar_fornecedor():
    ...
    for linha in cursor.fetchall():
        texto= f"Fornecedor: {linha[0]} \n"
        caixa_texto.insert("1.0", texto)
                             

    print('Dados de fornecedor!!!!')
...
entry_bairro.grid(row=2, column=1, padx=10, pady=10)

entry_ende = tk.Entry(janela, width=35, bd=2)
entry_ende.grid(row=3, column=1, padx=10, pady=10)
...
entry_cnpj.grid(row=6, column=1, padx=10, pady=10)

entry_produto = tk.Entry(janela, width=35, bd=2)
entry_produto.grid(row=7, column=1, padx=10, pady=10)

entry_categoria = tk.Entry(janela, width =35, bd=2)
entry_categoria.grid(row=8,column=1, padx=10, pady=10)

entry_lote= tk.Entry(janela, width =35, bd=2)
entry_lote.grid(row=9,column=1, padx=10, pady=10)



#Titulo Entradas:
label_nome = tk.Label(janela, text='Nome',justify="left", )
label_nome.grid(row=0,column=0, padx=5, pady=2, sticky="W", columnspan=1)

label_cidade = tk.Label(janela, text='Cidade', justify="left")
label_cidade.grid(row=1, column=0, padx=5, pady=2, sticky="W", columnspan=1)

label_bairro = tk.Label(janela, text='Bairro', justify="left")
label_bairro.grid(row=2, column=0 , padx=5, pady=2, sticky="W", columnspan=1)


label_ende = tk.Label(janela, text='Endereço', justify="left")
label_ende.grid(row=3, column=0, padx=5, pady=2, sticky="W",columnspan=1)

label_telefone = tk.Label(janela, text='Telefone', justify="left")
label_telefone.grid(row=4,column=0, padx=5, pady=2, sticky="W", columnspan=1)

label_email = tk.Label(janela, text='Email', justify="left")
label_email.grid(row=5,column=0, padx=5, pady=2, sticky="W", columnspan=1)

label_cnpj = tk.Label(janela, text='CNPJ',justify="left")
label_cnpj.grid(row=6,column=0, padx=5, pady=2, sticky="W", columnspan=1)

label_Produto = tk.Label(janela, text='Produto', justify="left")
label_Produto.grid(row=7,column=0, padx=5, pady=2, sticky="W", columnspan=1)

label_categoria = tk.Label(janela, text='Categoria', justify="left")
label_categoria.grid(row=8,column=0, padx=5, pady=2, sticky="W", columnspan=1)

label_lote= tk.Label(janela, text='Lote', justify="left")
label_lote.grid(row=9,column=0, padx=5, pady=2, sticky="W", columnspan=1)


botao_cadastrar = tk.Button(text='Cadastrar Fornecedor', command=cadastrar_fornecedor)
botao_cadastrar.grid(row=10, column=1,columnspan=1, padx=10, pady=10 , ipadx =5, sticky="nswe")

botao_Atualiza = tk.Button(text='Atualizar Fornecedor', command=atualizar_fornecedor)
botao_Atualiza.grid(row=11, column=1,columnspan=1, padx=10, pady=10 , ipadx =5, sticky="nswe")

botao_delete = tk.Button(text='Delete Fornecedor', command=deletar_fornecedor)
botao_delete.grid(row=12, column=1,columnspan=1, padx=10, pady=10 , ipadx =5, sticky="nswe")

botao_consulta = tk.Button(text='Consultar Fornecedor', command=consultar_fornecedor)
botao_consulta.grid(row=13, column=1,columnspan=1, padx=10, pady=10 , ipadx =5, sticky="nswe")

caixa_texto = tk.Text(janela, height=5, width=35, bd=2)
caixa_texto.grid(row=14, column=1)
